...
pretrack = function(filename,images,crop=c(0,0,0,0),rotang=0,filter,setmax=1,bgavg=5,diameter,separation=0,masscut,minimum,chan="grey",greyscale=FALSE,invert=FALSE,background="mean",fileext=".tif",ecccut=1,startimg=0){
  ...
  for (i in startimg:(images-1)){
    # Build the filepath and name for this image
    cat("Reading image ",i,"\n")
    thisimagename <- paste(filename,formatC(i,flag="0",digits=3),fileext,sep="")
    #cat(thisimagename,"\n")
    
    if (greyscale==FALSE){
      thisimage <- channel(readImage(thisimagename),chan)
    } else{
      thisimage <- readImage(thisimagename)
    }
    
    if (invert==TRUE){
      thisimage <- 1 - thisimage
    }
    ...
    # Lowpass this frame
    # if filter==0 then do not lowpass
    if (filter != 0){
      lp <- lowpass(image=thisimage,lobject=filter,bgavg=bgavg,background=background,setmax=setmax)
    } else{
      lp <- thisimage
    }
    ...
    # Find features
    f <- feature(image=lp,diameter=diameter,separation=separation,minimum=minimum,masscut=masscut,ecccut=ecccut)
